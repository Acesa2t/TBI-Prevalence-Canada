...
na.omit(tbi_2001%>%summarize(sum(NUMP)))
...
na.omit(tbi_2016%>%group_by(ISO3)%>%summarize(sum(LTBP)/sum(NUMP)))
# d <- na.omit(tbi_2016)%>%
#   mutate(AGE_BANDS = ifelse(
#     AGEP < 15, "0-14", ifelse(
#       AGEP >= 15 & AGEP < 35, "15-34", ifelse(
...
ggplot(a_2001, aes(x = YARP, y = NUMP, color = inc_band_per100k))+
