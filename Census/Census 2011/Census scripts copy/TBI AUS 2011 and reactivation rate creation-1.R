...
#View(na.omit(census)%>%group_by(ISO3)%>%summarize(sum(LTBP)/sum(NUMP)))

census2 <- na.omit(census)%>%mutate(AARP = YARP-YOBP)
saveRDS(census2, file = paste0(path.out, "AUScensus_2011_v10.rds"))
