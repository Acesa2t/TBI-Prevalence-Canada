...
census <- read.csv(paste0(path.in, "census_classified_2001_v4 copy.csv"), header = T)
census <- census%>%select(-X, -AGE_BANDS, -YARP_GROUPS)
colnames(census) <- c("ISO3", "BPLP", "YARP", "AGEP", "YOBP", "NUMP", "WHO_R","CNSY")
census <- census%>%filter(!ISO3%in%c("FLK", "GIB", "GLP", "GUF", "LIE", "MTQ", "SHN", "SPM", "TWN"))
View(census)
...
View(tbhaz.200rep%>%filter(iso3%in%c("PRT","RUS","RWA")))
write_csv(tbhaz.200rep%>%filter(iso3%in%c("PRT","RUS","RWA")), file = "/Users/ajordan/OneDrive - McGill University/LTBI-Aust-CEA-master/Census/Census 2001 Year/tb_hazards_CAN_2001.csv")
...
census_char <- apply(census,2,as.character)
write.csv(census_char,"/Users/ajordan/OneDrive - McGill University/LTBI-Aust-CEA-master/Data/Outputs/estimates_2001_iso3_v1.csv", row.names = FALSE)
    # This output is technically v5 but v3 was overwritten after an error, so now v4 despite input files saying v5

saveRDS(census, file = paste0(path.out, "estimates_2001_iso3_v1.rds"))

#' DATA OUTPUTS =======================================================================================================#

#' Save all the objects that haven't yet been saved.
saveRDS(prob.Inf, file = paste0(path.out, "2001.prob.Inf_iso3_v1.rds"))
View(readRDS("2001.prob.Inf_iso3_v1.rds"))
saveRDS(census, file = paste0(path.out, "2001_census_iso3_v1.rds"))
