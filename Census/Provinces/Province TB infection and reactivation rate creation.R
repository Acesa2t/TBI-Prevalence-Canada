...
census <- read.csv(paste0(path.in, "census_classified_other_provinces copy.csv"), header = T)
View(census)
census <- census%>%select(-X.1, -X, -AGE_GROUP, -YARP_group)
...
saveRDS(census, file = paste0(path.out, "estimates_other_provinces_v1.rds"))

#write.csv(census,
#          "/Users/ajordan/OneDrive - McGill University/LTBI-Aust-CEA-master/Data/Outputs/estimates_ontario_v1.csv", row.names = FALSE)
...
saveRDS(prob.Inf, file = paste0(path.out, "prob.Inf_other_provinces_v1.rds"))

saveRDS(census, file = paste0(path.out, "census_other_provinces_v1.rds"))
