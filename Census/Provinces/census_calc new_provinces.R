...
# Read in file
  ...
  census_rep <- read_xlsx("/Users/ajordan/Library/CloudStorage/OneDrive-McGillUniversity/LTBI-Aust-CEA-master/Census/Provinces/Other Provinces/census_rep_other_provinces.xlsx")
class(census_rep)
...
census_cols <- census_cols%>%
  mutate(age_group = ifelse(age >= 0 & age <=14, 1,
      ifelse(age >= 15 & age<=24, 2,
        ifelse(age >= 25 & age<=54,3,                                                     
               ifelse(age >= 55 & age<=64,4,                                                     
                      ifelse(age >= 65 & age<=85,5,6))))))
...
census_colsv2 <- census_colsv2%>%
  mutate(census_year = 2016, 
         YOBP = 2016-AGEP, 
         YARP_group = ifelse(YARP == 1980, 1,
                             ifelse(YARP >=1981&YARP<=1990, 2,
                                    ifelse(YARP>=1991&YARP<=2000,3,
                                           ifelse(YARP>=2001&YARP<=2010,4,
                                                  ifelse(YARP>=2011&YARP<=2016, 5, 6))))))
...
write.csv(census_colsv3, file = "/Users/ajordan/Library/CloudStorage/OneDrive-McGillUniversity/LTBI-Aust-CEA-master/Census/Provinces/Other Provinces/census_calced_other_provinces.csv")
