...
# This script distributes the population of migrants for all years except for Before 1961
...
census_colsv2 <- census_colsv2%>%mutate(YARP = ifelse(YARP == "Before 1961", 1960, as.numeric(YARP)))
#View(census_colsv2)
...
census_colsv2 <- census_colsv2%>%
  mutate(census_year = 2011, 
         YOBP = 2011-AGEP, 
         YARP_group = ifelse(YARP == 1960, 1,
                             ifelse(YARP >=1961&YARP<=1970, 2,
                                    ifelse(YARP>=1971&YARP<=1980,3,
                                           ifelse(YARP>=1981&YARP<=1990,4,
                                                  ifelse(YARP>=1991&YARP<=2000,5, 
                                                  ifelse(YARP>=2001&YARP<=2006, 6, 7)))))))
...
census_colsv3%>%ungroup()%>%summarise(tot_pop = sum(NUMP))
...
write.csv(census_colsv3, file = "/Users/ajordan/OneDrive - McGill University/LTBI-Aust-CEA-master/Census/Census 2006/2006_census_data_clean.csv")

# Next script census_extension copy.R
