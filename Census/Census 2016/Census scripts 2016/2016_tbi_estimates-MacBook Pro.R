...
View(na.omit(tbi)%>%
       group_by(WHO_R)%>%
       summarize(sum(NUMP)/7464250))

View(na.omit(tbi)%>%filter(AGEP<5)%>%group_by(WHO_R)%>%summarize(sum(NUMP)))

tbi_dist <- na.omit(tbi)%>%
  filter(ISO3 == "IND")%>%
  group_by(YARP)%>%
...
# Year and Inc Band
p1 <- ggplot( tbi_inc_plot, aes(YARP, tbi_prev*100, color = inc_band_per100k))+
  geom_smooth()+
  labs(
...
View(data.frame(build1$data))


ggplot( tbi_inc_plot, aes(YARP, num_pop, color = inc_band_per100k))+
  geom_smooth(se = FALSE)+
  labs(title = "Number Immigrating to Canada by TB Disease Incidence in COO",
       x = "Year of Immigration to Canada",
       y = "Number of Entrants",
       color = "TB Disease Incidence \n in Country of Origin \n per 100,000 Persons")+
...
tbi_inc_plot2 <- na.omit(tbi_age_groups)%>%
  group_by(ISO3, YARP, inc_band_per100k)%>%
  summarise(NUMP = sum(NUMP))


tbi_2016_top_10 <- unique(ungroup(tbi_inc_plot2)%>%
  select(-YARP, -NUMP)%>%filter(ISO3%in%c("CHN", 
                                               "VNM",
                                               "PAK",
                                               "IND",
                                               "PHL",
