...
#exclusions <- tbi_all_census_years%>%
...
mood.medtest(data=country_ltbp_combined_long,tbi_prev~CNSY)
```


```{r}

aarp_select <- tbi_all_census_years%>%
  filter(AARP_BANDS=="AARP 35-54"  & ISO3!="CAN")

ltbp_set <- as.data.frame(t(mapply("+",aarp_select$LTBP)))
View(ltbp_set)

aarp_ltbp_combined <- cbind(aarp_select,ltbp_set)

View(aarp_ltbp_combined)

#remove LTBP column
aarp_ltbp_combined_long <- aarp_ltbp_combined%>%
  select(-LTBP)%>%
  pivot_longer(cols = V1:V200,
               names_to = "former_col_names",
               values_to = "LTBP_values")%>%
...
View(aarp_ltbp_combined_long)

library(rstatix)
kruskal.test(data=aarp_ltbp_combined_long,tbi_prev~CNSY)
mood.medtest(data=aarp_ltbp_combined_long,tbi_prev~CNSY)
# aarp_ltbp_combined_long%>%
#   group_by(inc_band_per100k)%>%
#   kruskal_test(tbi_)

# ggplot(aarp_ltbp_combined_long, aes(tbi_prev))+
#   geom_histogram()+
#   facet_wrap(.~CNSY)

```

Probability of Infection Histogram
